...
function gf_SetCookie(name, value) {
    ...
    expires.setTime(expires.getTime() + 1000*60*60*24*30); // 30'days exist
...
*/
var gf_List2jsonWithLevel = function ( alist, arootId, aidCol, arootCol, aorderCol ) {
    var atreeModel = [];
        ...
        for ( var i=0, child; child = achildren[i]; i++ ) {
            if ( child[aidCol] === item[arootCol] ) {
                ...
                child.children.sort(function(a, b){ 
                    return a[aorderCol] < b[aorderCol] ? -1 : a[aorderCol] > b[aorderCol] ? 1 : 0;  
                });
                break;
            } else {
...
/**
* @function 	get wanted scope with handle (it could be property name or method name)
*/
function gf_Scope(scope, handle) {
    ...
    for (var i in scope) {
      ...
      if (typeof scope[i] == 'object' && scope[i] != null && scope[i].$id && checked.indexOf((scope[i].$id + ',')) == -1) {
      	checked += scope[i].$id + ',';
...
function gf_ScopeWithId(scope, handle) {
    ...
    for (var i in scope) {
      if (!scope.hasOwnProperty(i)) continue;
      if (typeof scope[i] == 'object' && scope[i] != null && scope[i].$id == handle) {
      	checked += scope[i].$id + ',';
      	//console.log(scope[i].$id + '/' + scope[i]);
    ...
    if(navigator.geolocation) {
        ...
        navigator.geolocation.getCurrentPosition(
            function successCallback(position) {
                ...
                rslt.coordinates = position.coords.latitude + "," + position.coords.longitude;
                callback(rslt);
            },
 ...
 * @type : function
 * @access : public
 * @desc : checking certain priv from session, a_buttonpriv: ex) "sto_bttn_able_yn"
 */
...
var gf_CreateFaviconImgUrl = function ($http, image_url, site, cb) {
  var param = { image_url: image_url, site: site }
  $http.post('/v1/faviconImgUrl', param).
  success(function(data, status, headers, config) {
  	data.filePath = document.location.origin + '/' + data.filePath;
    cb(data);
  }).
  error(function(data, status, headers, config) {
    alert("There was a problem loading the favicon.");
  });
}; 
